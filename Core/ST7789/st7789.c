...
{
	static uint16_t lineBuffer[ST7789_WIDTH];
	color=color<<8 | color>>8; //swapping bytes

	//filling framebuffer
	for(uint16_t *pWorking=&lineBuffer[0]; pWorking<&lineBuffer[ST7789_WIDTH]; ++pWorking){
		*pWorking=color;
	}
	...
	for(int j=0; j<ST7789_HEIGHT; ++j){
		ST7789_WriteData((uint8_t *)lineBuffer, sizeof(uint16_t)*ST7789_WIDTH);
	}
	ST7789_UnSelect();
}
